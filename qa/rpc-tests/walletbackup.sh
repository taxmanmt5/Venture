...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} evo"
...
function StartThree {
  $VentureD $VentureDARGS $B1ARGS &
  B1PID=$!
  $VentureD $VentureDARGS $B2ARGS &
  B2PID=$!
  $VentureD $VentureDARGS $B3ARGS &
  B3PID=$!
}
