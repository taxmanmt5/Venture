...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /c/deps/Venture-master/win32-build/src/Ventured.exe $OUT_DIR/Ventured.exe
  cp /c/deps/Venture-master/win32-build/src/test/test_Venture.exe $OUT_DIR/test_Venture.exe
  cp /c/deps/Venture-master/win32-build/src/qt/Ventured-qt.exe $OUT_DIR/Venture-qt.exe
  # Linux:
  cp /c/deps/Venture-master/linux-build/src/Ventured $OUT_DIR/Ventured
  cp /c/deps/Venture-master/linux-build/src/test/test_Venture $OUT_DIR/test_Venture
  cp /c/deps/Venture-master/linux-build/src/qt/Ventured-qt $OUT_DIR/Venture-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /c/deps/Venture-master/linux-build
make check

# Run RPC integration test on Linux:
/c/deps/Venture-master/qa/rpc-tests/wallet.sh /c/deps/Venture-master/linux-build/src
/c/deps/Venture-master/qa/rpc-tests/listtransactions.py --srcdir /c/deps/Venture-master/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /c/deps/Venture-master/win32-build
  make check
fi
