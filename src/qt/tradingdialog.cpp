    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->VCAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(VC)"<< "VC(SIZE)" << "BID(VC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(VC)"  << "VC(SIZE)" << "TOTAL(VC)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(VC)"<<"TOTAL COST(VC");
...
void tradingDialog::UpdaterFunction(){
    //VCst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=evo-VC");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=VC-VC&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=VC-VC&count=100");
      return Response;
}
...
QString tradingDialog::BuyVC(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=VC-VC&quantity=";
...
QString tradingDialog::SellVC(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=VC-VC";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=VC";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=VC-VC&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->Ask->setText("<b>Ask:</b> <span style='font-weight:bold; font-size:14px; color:Red'>" + str.number(obj["Ask"].toDouble(),'i',8) + "</span> VC");

    ui->Bid->setText("<b>Bid:</b> <span style='font-weight:bold; font-size:14px; color:Green;'>" + str.number(obj["Bid"].toDouble(),'i',8) + "</span> VC");

    ui->volumet->setText("<b>VC Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> VC");

    ui->volumeevo->setText("<b>VC Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["BaseVolume"].toDouble(),'i',8) + "</span> VC");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        ui->VCSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(VCSupply,'i',8) + "</span><b> VC</b>");
        ui->EvoSupply->setText("<span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(EvoSupply,'i',8) + "</span><b> VC</b>");
        ...
        ui->VCDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(VCDemand,'i',8) + "</span><b> VC</b>");
        ui->EvoDemand->setText("<span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(EvoDemand,'i',8) + "</span><b> VC</b>");
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       ui->VCAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("VC");
                         if(Response.size() > 0 && Response != "Error"){
                          DisplayBalance(*ui->VentureBalanceLabel,*ui->VentureAvailableLabel,*ui->VenturePendingLabel, QString::fromUtf8("VC"),Response);
                         }

                         Response = GetBalance("VC");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->VCBalanceLabel,*ui->VCAvailableLabel,*ui->VCPendingLabel, QString::fromUtf8("VC"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputVC->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of VC that can be gained from selling VC available balance
    QString responseA = GetBalance("VC");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableVC = ResultObject["Available"].toDouble();

    ui->UnitsInputVC->setText(str.number(AvailableVC,'i',8));
}

void tradingDialog::on_Buy_Max_Amount_clicked()
{
    //calculate amount of currency than can be brought with the VC balance available
    QString responseA = GetBalance("VC");
    ...
    double Result = (AvailableVC / CurrentASK);
...
void tradingDialog::on_BuyVC_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyVC(Order,Quantity,Rate);
...
void tradingDialog::on_SellVCBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputVC->text().toDouble();
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellVC(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputVC_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
