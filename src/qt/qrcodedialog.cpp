...
{
    QString ret = QString("Venture:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VC unit input in lnReqAmount, we generate the URI with VC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(VentureUnits::format(VentureUnits::VC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
